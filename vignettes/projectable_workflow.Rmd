...
crs_tab_col <- mtcars %>% 
  set_table(
    ...
    .cols = list(
      `V-Shaped` = encol_freq(n = vs %in% 1, N = mtcars$vs %in% 1), 
      `Not V-Shaped` = encol_freq(n = vs %in% 0, N = mtcars$vs %in% 0)
    )
  )
...
crs_tab_overall <- mtcars %>% 
  set_table(
    ...
    .cols = list(
      `V-Shaped` = encol_freq(n = vs %in% 1, N = nrow(mtcars)), 
      `Not V-Shaped` = encol_freq(n = vs %in% 0, N = nrow(mtcars))
    )
  )
